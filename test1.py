...
warehouse_chaos = 0
warehouse_divine = 0
chaos_label = None  # 全局變數
divine_label = None  # 全局變數
tree = None  # 全局變數
    ...
    if os.path.exists(DATA_FILE):
        try:
            with open(DATA_FILE, "r", encoding="utf-8") as f:
                ...
                current_chaos = data.get("current_chaos", 100.0)
                current_divine = data.get("current_divine", 100.0)
                # 確保每個項目包含所有必要的鍵
                for item in items:
    ...
    if column in column_mapping:
        ...
        if new_value_str is not None:
            try:
                ...
                profit_c_to_d = (items[selected_index]['divine_sell_price'] * items[selected_index]['dc_ratio']) - items[selected_index]['receive_price']
                ...
                items[selected_index]['purchasable_with_divine'] = int((current_divine * items[selected_index]['dc_ratio']) // items[selected_index]['receive_price']) if items[selected_index]['receive_price'] > 0 else 0
...
# 初始化倉庫混沌石和神聖石數量，每次開啟程式時歸零
def reset_resources():
    global warehouse_chaos, warehouse_divine
    warehouse_chaos = 0
    warehouse_divine = 0
    chaos_label.config(text=f"倉庫混沌石數量: {warehouse_chaos}")
    divine_label.config(text=f"倉庫神聖石數量: {warehouse_divine}")

# 手動輸入混沌石和神聖石數量
# 使用 `parent=root` 在更新資源時
def update_resources():
    global current_chaos, available_divine_stones, tree

    try:
        # 讓用戶輸入混沌石和神聖石的數量
        new_chaos_str = simpledialog.askstring("輸入混沌石數量", "請輸入目前混沌石數量:")
        new_divine_str = simpledialog.askstring("輸入神聖石數量", "請輸入目前神聖石數量:")
        ...
        available_divine_stones = new_divine  # 更新背包中的神聖石
        save_items_to_file()
        ...
        for index, item in enumerate(items):
            ...
            # 將背包中的神聖石轉換為混沌石，再計算可購買數量
            total_chaos_from_divine = available_divine_stones * dc_ratio
        ...
        # 更新顯示的可購買數量
        chaos_quantity_label.config(text=f"可購買混沌石數量: {int(current_chaos)}")
        divine_quantity_label.config(text=f"背包神聖石數量: {int(available_divine_stones)}")

        messagebox.showinfo("更新成功", f"當前混沌石數量: {current_chaos}, 當前神聖石數量: {available_divine_stones}")
    except ValueError:
        messagebox.showerror("錯誤", "請輸入有效的數字。")

# 計算並顯示利潤
def calculate_profit():
    global tree, items, current_chaos, available_divine_stones  # 確保 available_divine_stones 是全局變數
    ...
    try:
        divine_buy_price = 1 / parse_fractional_input(fields["神聖石購買價格"])
    except ValueError:
        messagebox.showerror("錯誤", "神聖石購買價格輸入錯誤。")
        return

    try:
        divine_sell_price = 1 / parse_fractional_input(fields["神聖石販賣價格"])
    except ValueError:
    ...
    # 計算可購買數量（使用背包中的神聖石）
    total_chaos_from_divine = available_divine_stones * dc_ratio  # 確保這裡使用了全局變數
    purchasable_with_chaos = int(current_chaos // receive_price) if receive_price > 0 else 0
    purchasable_with_divine = int(total_chaos_from_divine // receive_price) if receive_price > 0 else 0
    ...
    item_data = {
        ...
        "purchasable_with_divine": purchasable_with_divine  # 這裡用轉換後的混沌石數量計算
    }
...
def main():
    ...
    tree.heading("purchasable_with_chaos", text="倉庫混沌石")
    tree.heading("purchasable_with_divine", text="倉庫神聖石")
